const getWindows = () => {
  ...
  const active = exec('chunkc tiling::query --window owner').trim()
  const windows = parse(exec('chunkc tiling::query --desktop windows'))
  return windows.map(({ id, owner, title }) => {
    return {
      id, owner, title, 
      isActive: owner.trim() === active
    }
  }).filter(window => window.title !== "(invalid)").sort((a, b) => a.owner.localeCompare(b.owner))
}
